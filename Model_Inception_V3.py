...
class StyleTransfer:
  def __init__(self, content_image, style_image, content_layers, style_layers):
    ...
    self.inceptionV3 = tf.keras.applications.InceptionV3(include_top=True, weights="imagenet", input_tensor=None, \
                  input_shape=None, pooling=None, classes=1000, classifier_activation="softmax")
    self.init_rngs = {"params": jax.random.PRNGKey(0)}
    self.resparams = self.inceptionV3.init(self.init_rngs, content_image)
    self.fn_out = self.inceptionV3.apply
